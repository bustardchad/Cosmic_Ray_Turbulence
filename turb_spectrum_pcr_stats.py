...
def doit(ds,i):
 ...
 #   ref = int(np.product(ds.ref_factors[0:max_level]))
   ...
   # for vel in [("gas", "magnetic_field_y")]:
    ...
    Kk = fft_comp(ds, ("athena_pp","Ec"), max_level, low, dims)
    # wavenumbers
...
ts = yt.DatasetSeries("../cr.out1.0009*")
i = 0
EkinVector = []
EkinrhoVector = []
EthermVector = []
...
a = ekvec[1:7,:]  # last few rows (times) of spectrum array
...
plt.ylabel(r"FFT of CR Compression Rate",fontsize=18)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(loc="lower right")
plt.tight_layout()
plt.savefig("compression_spectrum_streaming_res256_pcpg1_varybeta.pdf")
plt.close()
