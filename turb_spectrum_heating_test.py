...
def doit(ds,i):
 ...
 #   ref = int(np.product(ds.ref_factors[0:max_level]))
   ...
   #     E_spectrum[n-1] = np.sum(Kk.flat[whichbin==n])
    # sum over bins in k -- this is the integral part
    for n in range(1,len(ncount)):
        print(Kk.flat[whichbin==n])   
        E_spectrum[n-1] = np.sum(Kk.flat[whichbin==n])
...
def fft_comp(ds, iu, level, low, delta ):
   ...
   # ru = np.fft.fftn(rho**nindex_rho * u)[0:nx//2+1,0,0]
    ru = np.fft.fftn(u)[0:nx//2+1,0:ny//2+1,0:nz//2+1] # take sqrt so returned value has units of dE/dt x k^-1
