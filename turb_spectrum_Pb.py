...
def doit(ds,i):
 ...
 #   ref = int(np.product(ds.ref_factors[0:max_level]))
   ...
   # for vel in [("gas", "magnetic_field_y")]:
    ...
    Kk = fft_comp(ds, ("gas","magnetic_energy_density"), max_level, low, dims)
...
a = ekvec[1:8,:]  # last few rows (times) of spectrum array
...
plt.loglog((k), avg*(k**2), 'bo-',label=r"MHD, $\beta \sim 1$")
...
plt.ylabel(r"FFT of $P_{B}$",fontsize=18)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(loc="lower right")
plt.tight_layout()
plt.savefig("mag_spectrum_MHD_res512_beta1.pdf")
plt.close()
